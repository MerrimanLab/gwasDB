...
load_markers <- function(chrom) {
  markers <- vroom::vroom(file = path("~/data/ukbb500k_gwas/db_load/",paste0("marker_ids_chr",chrom,".txt")), col_names = c("kgp_id","snp","chr","pos","ref","alt"))
  # load into b37 table
...
load_gwas <- function(chr, studyid) {
  # read gwas
  gwas <- read_tsv(path("~/data/ukbb500k_gwas/",paste0("gout_gwas_chr",chr,"_add_unfiltered_p.tsv"))) %>% janitor::clean_names()
  
  # read impute marker info
  impute_info <- vroom::vroom(path("~/data/ukbb500k_gwas/",paste0("ukb_mfi_chr",chr,"_v3.txt")), col_names = c("chr_pos_alleles","snp_id", "position", "ref","alt","maf", "a1","info_score")) %>% mutate(remove_snp = ifelse(info_score < 0.3, TRUE, FALSE))
  
  # read hwe info
  message("pivoting")
  # pivot wide
  wide_hwe <- read_tsv_chunked(path("~/data/ukbb500k_gwas/",paste0("chr",chr,".hwe.tsv")), callback = DataFrameCallback$new(pivot_hwe), chunk_size = 90000, progress = TRUE) 
